...
if __name__ == "__main__":
    ...
    base_model_path = '/home/one/pretrained_model/Qwen2.5VL-7B-instruct'
    ...
    gpu_ids = [1, 2, 3, 4]  # 根据你机器上的 GPU 数量设定

    processes = []
    for i, args in enumerate(task_args):
        p = Process(target=worker, args=(args, gpu_ids[i % len(gpu_ids)]))
        p.start()
        processes.append(p)
